...
export function GoalsView() {
    ...
    const plan = simulatePlan( buildPlanLines( goals, accounts, all.loans ), surplusOf( all.settings ) );
    ...
    const card = ( p: ( typeof progress )[ number ] ) => <GoalCard key={ p.goal.id } p={ p } doneMonth={ plan.outcomes[ p.goal.id ]?.doneMonth ?? null } />;
    ...
    return (
        <div>
            ...
            { progress.length === 0 && (
                <Empty icon={ <Target size={ 36 } /> } title="No goals yet" body="Start with job-loss cover. Then something you want — a trip, a phone, a gift for Mom — with a date." action={ <AddGoalButton accounts={ linkable } goals={ goalOpts } /> } />
            ) }
